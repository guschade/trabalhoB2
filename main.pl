...
dijkstra([Custo-V-[V|T] | Resto], Visitados, Destino, Caminho, CustoFinal) :-
  findall(CustoNovo-W-[W,V|T], (aresta(V, W, Peso), \+ member(W, Visitados), CustoNovo is Custo + Peso), Novos), append(Resto, Novos, Total), sort(Total, TotalOrdenado),
...
dijkstra_todos_caminhos(Origem, Destino, Caminhos) :-
    findall(Caminho-Custo, dijkstra(Origem, Destino, Caminho, Custo), CaminhosNaoOrdenados), sort(2, @=<, CaminhosNaoOrdenados, Caminhos).
...
bellman_ford(Origem, Destino, Caminho, Custo) :-
    inicializar_distancias(Origem), vertice_num(N), atualizar_distancias(N),
\+ existe_ciclo_negativo, caminho_minimo(Origem, Destino, CaminhoInvertido, Custo),
reverse(CaminhoInvertido, Caminho).
...
atualizar_distancias(N) :-
    findall(_, (aresta(U, V, Peso), relaxar(U, V, Peso)), _), N1 is N - 1, atualizar_distancias(N1).

% Função para relaxar uma aresta
relaxar(U, V, Peso) :-
    distancia(U, DistU), DistU \= inf, distancia(V, DistV), NovoDistV is DistU + Peso,
(NovoDistV < DistV -> retract(distancia(V, DistV)), assert(distancia(V, NovoDistV)),
retractall(predecessor(V, _)), assert(predecessor(V, U)); true).

% Verificação de ciclos negativos
existe_ciclo_negativo :-
    aresta(U, V, Peso), distancia(U, DistU), DistU \= inf, distancia(V, DistV), NovoDistV is DistU + Peso, NovoDistV < DistV.

% Função para encontrar o caminho mínimo
caminho_minimo(Origem, Destino, [Destino | Caminho], Custo) :-
    distancia(Destino, Custo), caminho_minimo_aux(Origem, Destino, Caminho).

caminho_minimo_aux(Origem, Origem, []).
caminho_minimo_aux(Origem, V, [U | Caminho]) :-
    predecessor(V, U), caminho_minimo_aux(Origem, U, Caminho).

% Função para contar o número de vértices
vertice_num(N) :-
    findall(V, vertice(V), Vertices), length(Vertices, N).
...
encontrar_caminhos_alternativos_bf(Origem, Destino, Caminhos) :-
    inicializar_distancias(Origem), vertice_num(N), atualizar_distancias(N),
\+ existe_ciclo_negativo, findall(CaminhoInvertido-Custo, (distancia(Destino, Custo),
caminho_minimo(Origem, Destino, CaminhoInvertido, Custo), \+ member(CaminhoInvertido, Caminhos)), CaminhosNaoOrdenados), sort(2, @=<, CaminhosNaoOrdenados, Caminhos).

% Função para mostrar caminhos alternativos com Bellman-Ford
mostrar_caminhos_alternativos_bf(Origem, Destino) :-
    encontrar_caminhos_alternativos_bf(Origem, Destino, Caminhos), writeln('Caminhos alternativos:'), mostrar_caminhos(Caminhos).

% Função auxiliar para mostrar a lista de caminhos
mostrar_caminhos([]).
mostrar_caminhos([Caminho-Custo | Resto]) :-
    reverse(Caminho, CaminhoInvertido), format('Caminho: ~w, Custo: ~w~n', [CaminhoInvertido, Custo]), mostrar_caminhos(Resto).
...
% Remoção de todas as arestas conectadas a um vértice
remove_arestas_vertice(Vertice) :-
    retractall(aresta(Vertice, _, _)),
    retractall(aresta(_, Vertice, _)).

% Remoção de um vértice e todas as suas arestas
